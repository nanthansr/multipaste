...
class ClipboardManager {
    ...
    private func checkPasteboard() {
        ...
        if types.contains(where: { skipTypes.contains($0) }) {
            fileLog("Skipping concealed/transient pasteboard change")
            return
        }
        ...
        fileLog("Pasteboard changed to count: \(currentChangeCount)")
        
        if isPasting {
            // Skip capturing if we are currently injecting a paste
            fileLog("Skipping capture because isPasting is true")
            return
        }
        ...
        if let text = pasteboard.string(forType: .string) {
            ...
            fileLog("No string found in pasteboard")
        }
    }
